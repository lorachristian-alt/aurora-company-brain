...
⚠️ **RECEPIMENTO DEL GATE DI 3E: UN'ESPRESSIONE DEL DOMINIO `ritiro` È RESPINTA, E LA RESPINTA
VIAGGIA CON LA DICHIARAZIONE.** La metà B del collaudo di E59, eseguita a lotto chiuso, ha tolto
`\bPRO-QA-11\b` con **quota fuori 0,57**: è la sigla che `PRO-QA-08` usa, e pescava le note del
dominio `reclami`. **Alla rigenerazione dei due domini quell'espressione NON rientra senza una
prova nuova** — chi la rimettesse dentro rifaà prima la prova B e ne dichiara l'esito nel rapporto
di R2.

⚠️ **È scritto anche in testa a `qa\lotti\r2_reclami_verticale.txt`, che è la riga che R2 legge in
apertura**, ed è la forma di **E62**: un esito che vive solo in un rapporto chiuso non lo vede chi
apre il lotto sei giorni dopo, e l'espressione tornerebbe dentro per la via più naturale del
...
| **24/08/2026** | **gate 3E** | ⚠️ **Lo spezzamento è RATIFICATO**, e con esso il fatto che a chiudere il tema 3 sia `3F` e non `3E`: la matrice lo registra, e ora sono **33 elenchi**. ⚠️ **Il piano del prompt del gate diceva il contrario, e a smentirlo è stata la conta dei fatti in apertura** — 62 su due grezzi, oltre il tetto che E28 non lascia scegliere. Da qui il corollario alla regola del coordinatore: **una pianificazione scritta in un prompt porta il condizionale della misura** (« ultimo pacchetto, salvo il conto di E21 »), perché un'apertura può sempre smentire un piano ed è fatta apposta. ⚠️ **E il RINOMINO dell'elenco è ratificato come precedente**: `lotto_03e_crisi_ispezioni` → `lotto_03e_crisi_ritiro` è **E30 esteso applicato a un elenco** invece che a una nota, ed è la prima volta che quella regola esce dal vault. ⚠️ **R2 a due domini: ratificato**, e la respinta di `\bPRO-QA-11\b` è scritta dove R2 la legge (sopra). ⚠️ **Nessun grezzo si sposta, e nessuna riga T nasce da questo gate**: il debito resta **T171-T175**, aperte dichiarate. **Il prossimo atto è `3F`**, poi `R2` in sessione nuova |

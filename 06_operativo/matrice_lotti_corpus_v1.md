...
| T126 | **Il vault dava il ritardo delle evidenze a SEDICI giorni, e i documenti dell'ente lo danno a QUINDICI**: il termine e' il **18/03/2026** — «28 gg dalla riunione di chiusura» — e il verbale interno lo trascrive al **17/03** contandolo «dalla notifica» | **lotto 3C** — correzione di una nota gia' scritta | **entrambe le gambe canonizzate**: il verbale dal lotto 3A, i tre documenti dell'ente da questo | **riconciliata** — ⚠️ **Non e' un errore di nessuno dei due**: entrambi i conti fanno ventotto giorni, cambia il punto di partenza. **Vale il termine dell'ente, che e' chi lo impone**, e la nota `fatto-evidenze-audit-oltre-termine` e' stata corretta nel titolo, nel summary e nel corpo, con le tre fonti a confronto ⚠️ **RIVISTA nello stesso lotto, dal terzo giro di giudizio: il «sedici» NON era un refuso del vault.** Il registro delle non conformita' interne porta `NC-2026-061` del 02/04/2026 — «Invio a CSQA evidenze chiusura NC audit **con 16 gg di ritardo** sulla scadenza» — quindi **Aurora conta sedici in DUE documenti**, il verbale e il registro, ed e' coerente con se' stessa. **Il conto dell'ente resta quindici**, perche' il termine e' suo e lo fissa al 18/03. ⚠️ **La prima stesura di questa riga diceva che il vault era «sbagliato»: non lo era, contava da un altro giorno.** |
...
| T129 | **Il tasso di difetto di produzione del lotto 3C e' 38,7 %, il piu' alto della serie**, e dodici note su trentuno lo compongono — ma **otto di quelle dodici citano un'altra fonte prescrittiva** che governa cio' di cui parlano davvero (il manuale HACCP, il registro delle tarature), e delle quattro restanti **tre hanno la propria prescrizione FUORI dal corpus**, nel protocollo BRCGS, e una in una «procedura interna» che l'archivio non ha | **lotto 3C** — misura di apertura, non divergenza documentale | **il gate**, che deve decidere se il dominio `certificazione` sia dichiarato troppo largo | **aperta dichiarata** — ⚠️ **Il numero NON e' stato aggiustato** (E41): nessuna fonte e' stata aggiunta per abbassarlo, e le **quattro correzioni verticali vere** che il lotto ha fatto — `MOD-PR-04` sui ricambi, `PRP-05`/`PRP-12` sul vendor rating, `PRP-02` sugli infestanti, `PRP-04` sulla formazione — **non lo toccano affatto**, perche' riguardano il dominio del manuale HACCP e non quello dichiarato. ⚠️ **Ed e' questo il punto**: le ESPRESSIONI del dominio (`BRCGS`, `IFS`, `grade`, `audit di rinnovo`) riconoscono le note che parlano **dell'audit**, mentre le sue FONTI governano **il titolo e gli obblighi verso l'ente**. E' l'errore che E36 corresse per le note, ricomparso nella dichiarazione di un dominio |
...
| T141 | ⚠️ **DUE NOTE DEL LOTTO NON SONO PASSATE DAL GIUDIZIO**, e sono quelle nate dai ritrovamenti del **terzo** giro: `fatto-due-nc-interne-sul-proprio-ritardo` e `questione-vendor-rating-2025-c-e-o-non-c-e` | **lotto 3C** — debito di processo dichiarato | **il gate**, e il prossimo lotto che produca note post-revisione | **aperta dichiarata** — ⚠️ **La regola d'arresto E26 ha la precedenza**: il ciclo si ferma comunque al terzo giro, e il lotto si chiude **dopo che il rapporto ha nominato il pattern**, non ripetendo il ciclo. ⚠️ **Le due note hanno passato QA e controllo delle citazioni, non lo strato di giudizio**, e la differenza va detta: il primo verifica la forma, il secondo verifica che la nota non affermi oltre le proprie fonti. ⚠️ **E' lo stesso debito di T117, di specie diversa**: li' era la soglia di spezzamento, qui e' il giudizio |
| T142 | **La classe d'errore che il terzo giro ha cercato perche' gliel'hanno nominata: IL SUPERLATIVO SULL'ARCHIVIO.** «e' la sola parte dell'archivio in cui...», «e' il solo documento dell'archivio a nominarla», «e' il termine piu' stretto che questo archivio conosca», «e' l'unico riscontro in archivio» — quattro affermazioni che **nessuna fonte citata puo' reggere**, perche' parlano di cio' che l'archivio contiene ALTROVE | **lotto 3C** — nominata al secondo giro, cercata al terzo | **il gate**: e' un candidato emendamento | **aperta dichiarata** — ⚠️ **La regola non e' «niente superlativi»**: il terzo giro ne ha verificati **quattordici** e ne ha confermati **dieci**, tutti quelli che parlano di **un documento citato** — «e' l'unica cosa che questo verbale chiami obiettivo primario» e' vero e verificabile. ⚠️ **Il discrimine e' il SOGGETTO**: un superlativo su un documento fra le fonti si verifica; **uno sull'archivio non e' verificabile da nessuna nota**, perche' nessuna nota ha l'archivio fra le proprie fonti. ⚠️ **Ed e' la stessa forma di E36**, un gradino piu' su: l'affermazione eccede non il documento ma il perimetro |

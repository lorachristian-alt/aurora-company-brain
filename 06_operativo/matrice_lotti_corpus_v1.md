...
| T99 | **Come sia composto «il lavaggio completo CIP».** `IO-05` §5 elenca **sei voci** sotto la frase «il ciclo completo ha **5** fasi» — il conto torna solo escludendo la sanificazione, che e' condizionata al programma; la scheda allergeni §5.3 definisce l'`L3` «completo CIP secondo `IO-05`» **omettendo il prerisciacquo e includendo la sanificazione**. ⚠️ **Non e' «IO-05 piu' una fase»: e' IO-05 meno la prima e piu' l'ultima**, e il log le esegue tutte e sei mettendo la sanificazione dove nessuno dei due la elenca | **revisione col canone del lotto 2B-bis** | **tutte le gambe sono canonizzate**: `IO-05` e il log dal lotto 2A, la scheda da questo | **aperta dichiarata** — `questione-composizione-lavaggio-completo`. ⚠️ **RIAPRE UN ARBITRATO GIA' SCRITTO NEL CANONE.** Il gruppo del lotto 2A arbitrava «`IO-05`, e il log resta com'e'», concludendo che il tracciato fosse **piu' severo del nome che porta**. **Non regge come scritto**: la fase in piu' **ha una fonte prescrittiva in vigore che la chiede**, e la divergenza cambia specie — da *etichetta che non corrisponde al contenuto* a *due prescrittivi che non concordano*. La riga del canone porta ora il rimando |
| T100 | **La nota alla matrice spiega una casella che la matrice non contiene**: «"`PC` soia" su **Linea 1 e Linea 2**» — ma nessuna referenza di Linea 2 porta `PC` sulla soia (due `C`, una `A`) | **revisione col canone del lotto 2B-bis** | **nessuna: e' intra-file** | **aperta dichiarata** — dentro `questione-precauzionale-af-sn-0450-soia`, che quella riga la citava per intero **senza notarlo**. ⚠️ Stessa specie del riepilogo della scheda di manutenzione (18/08) e delle due date della procura (19/08) |

...
| T71 | **`MOD-QA-19`, il piano di monitoraggio dei tamponi di superficie**, è prescritto da `IO-05` §8 come quarto criterio di accettazione del lavaggio — «di norma 2 punti a settimana per linea, di più dopo le produzioni con allergeni» | **lotto 2A** | **lotto 2B**, che porta il registro dei tamponi di superficie | tracciata — ⚠️ **obbligo esplicito per il lotto 2B**: al momento di canonizzare il registro dei tamponi va confrontato con la frequenza che `IO-05` prescrive, e va verificato se i punti campionati coprano la Linea 1 dopo i lavaggi di maggio |
| T72 | **Il criterio di conducibilità di `IO-05` è DIFFERENZIALE e il suo termine di paragone non è nel lotto 2A**: «≤ 50 µS/cm sopra il valore dell'acqua di rete», e il log del CIP non misura mai l'acqua di rete. `kpi-conducibilita-risciacquo-cip-maggio` lo dichiara **non verificabile sulle proprie fonti**, e si ferma lì | **lotto 2A** — la conducibilità di rete è stata **vista in ricerca e NON scritta** (E25): sta in un grezzo del lotto 2B, e finché quel lotto non la porta nel vault non se ne scrive nulla, nemmeno in forma attenuata | **lotto 2B**, che porta il piano di autocontrollo dell'acqua potabile | tracciata — ⚠️ **obbligo esplicito per il lotto 2B**: quel piano misura la conducibilità al punto di ingresso della rete, cioè **esattamente il termine che manca**. Alla sua canonizzazione la nota della conducibilità va riaperta e il criterio di accettazione va finalmente applicato ai 28 cicli chiusi `PASS`. ⚠️ È la riconciliazione verticale **al contrario**: qui non manca la fonte che prescrive — c'è, ed è `IO-05` — manca **il dato che rende applicabile la prescrizione** |
